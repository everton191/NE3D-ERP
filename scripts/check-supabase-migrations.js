...
const checks = [
  ...
  ["diagnostics error logs updated_at repair", /alter table public\.app_error_logs[\s\S]*add column if not exists updated_at timestamptz not null default now\(\)/i],
  ["diagnostics affected users updated_at repair", /alter table public\.app_error_log_users[\s\S]*add column if not exists updated_at timestamptz not null default now\(\)/i],
  ["diagnostics anonymous feedback policy repair", /create policy "diagnostics feedback insert own"[\s\S]*with check \(user_id is null or user_id = auth\.uid\(\)\)/i],
  ["security advisor search path hardening", /alter function public\.set_updated_at\(\) set search_path = public, pg_catalog/i],
  ["security advisor trigger RPC revoke", /revoke all on function public\.diagnostics_set_updated_at\(\) from public, anon, authenticated/i],
  ["security advisor cleanup RPC revoke", /revoke all on function public\.s3d_cleanup_subscription_state\(\) from public, anon, authenticated/i],
  ["public asset listing disabled", /drop policy if exists "simplifica_assets_public_read" on storage\.objects/i],
