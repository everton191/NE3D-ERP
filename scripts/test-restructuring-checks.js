...
assert(/const APP_VERSION = "1\.0\.62-rc"/.test(app), "app.js esta na versao 1.0.62-rc");
assert(/const APP_VERSION_CODE = 61/.test(app), "app.js possui versionCode 61");
...
assert(/const CACHE_NAME = "simplifica-3d-v\d+-[^"]+";/.test(sw), "service worker possui cache versionado atual");
...
assert(/\/app\.js\?v=1\.0\.62-rc-[^"]+/.test(index), "index.html usa cache-bust atual");
