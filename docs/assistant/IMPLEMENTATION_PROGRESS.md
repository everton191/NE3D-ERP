...
- [x] ETAPA 46 — Histórico real do Caixa por conta. A versão 1.0.37/65 hidrata `erp_records` e backup no login, pagina até 10.000 registros por conta, une legado e movimentos canônicos sem duplicidade e alimenta Caixa, cards, extrato, IA e gráficos pela mesma fonte. O banco confirmou 41 movimentos distribuídos em 4 contas; testes, builds, instalação ADB, feed público e produção Vercel passaram. A entrada manual autenticada em cada uma das quatro contas continua como aceite manual.
...
## Evidência da publicação 1.0.34
...
## Evidência do runtime universal por aplicativo

- `AppAssistantRuntime` rejeita packs cujo `modelScope` não seja igual ao `manifest.appId`, impedindo compartilhamento acidental de configurações e artifacts.
- Cada pack fornece `createRuntime()` e contratos explícitos que ligam nomes de tools somente aos adapters do próprio aplicativo.
- `test:assistant-multi-app-runtime` criou Simplifica 3D, Rural, Tec e Editor da Loja sobre o mesmo armazenamento e confirmou quatro chaves distintas de conversa, cache e seleção de modelo.
- O teste gravou o mesmo fato e o mesmo identificador de cache nos quatro apps, recriou os runtimes e comprovou que cada um recuperou somente o próprio valor.
- Rotas cruzadas foram bloqueadas, READ retornou apenas dados do adapter correto, processamento remoto permaneceu bloqueado e o Editor da Loja não expôs escrita.
